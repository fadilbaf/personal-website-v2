...
export default function EmailTemplatesPage() {
  ...
  // Dynamic variable values per template ID
  ...
  // Sidebar parameters panel collapse toggle for mobile/desktop
  ...
  const renderedHtml = useMemo(() => {
    try {
      return activeTemplate.renderHtml(currentVariables, templateLocale);
    } catch (err) {
      console.error("Template render error:", err);
      return `<div style="padding: 20px; color: red;">Error rendering template: ${String(err)}</div>`;
    }
  }, [activeTemplate, currentVariables, templateLocale]);
  ...
  // Reset variables of current template to default
  const handleResetVariables = () => {
    ...
    toast.success(
      language === "id"
        ? "Variabel templat berhasil direset ke default"
        : "Template variables reset to defaults"
    );
  };

  // Copy active content (HTML or Plain Text)
  const handleCopyContent = async () => {
    try {
      const textToCopy = viewMode === "html" ? renderedHtml : renderedPlainText;
  ...
  return (
    <div className="space-y-6 max-w-[1600px] mx-auto pb-12">
      ...
      <div className="space-y-4">
        ...
        {/* Split View: [ Preview / Code Canvas ] + [ Variable Customizer Panel ] */}
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          ...
          >
            <div className="bg-neutral-950 rounded-2xl border border-neutral-800/80 shadow-2xl overflow-hidden min-h-[640px] flex flex-col">
                  ...
                  <span className="text-[11px] text-neutral-500 font-mono">
                    {viewMode === "preview"
                      ? viewport === "desktop"
                        ...
                        : "375px Mobile View"
                      : viewMode === "html"
                      ? `${(renderedHtml.length / 1024).toFixed(1)} KB HTML`
              ...
              <div className="flex-1 p-4 md:p-8 flex items-center justify-center bg-[#070809] overflow-auto">
                {/* 1. PREVIEW TAB */}
                {viewMode === "preview" && (
                  <div
                    className={cn(
                ...
                {/* 3. RAW HTML TAB */}
                {viewMode === "html" && (
                  <div className="w-full max-w-4xl bg-neutral-900/90 border border-neutral-800 rounded-xl p-6 overflow-x-auto shadow-xl">
                    <pre className="font-mono text-xs text-neutral-300 leading-normal select-text whitespace-pre">
                      <code>{renderedHtml.trim()}</code>
                    </pre>
                  </div>
                )}
              </div>
      ...
      <Dialog open={isTestModalOpen} onOpenChange={setIsTestModalOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              {language === "id"
                ? `Kirim templat "${activeTemplate.name}" dengan parameter saat ini ke kotak masuk email Anda.`
                : `Deliver a live test rendering of "${activeTemplate.name}" to your inbox.`}
            </DialogDescription>
          </DialogHeader>
